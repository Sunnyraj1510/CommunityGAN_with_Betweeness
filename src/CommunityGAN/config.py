...
class Config():
    ...
    def update_path(self):
        self.train_filename = "/content/CommunityGAN/data/" + self.app + "/" + self.dataset + "_train.txt"
        self.pretrain_emb_filename_d = "/content/CommunityGAN/pre_train/" + self.app + "/" + self.dataset + "_pre_train.emb"
        self.pretrain_emb_filename_g = "/content/CommunityGAN/pre_train/" + self.app + "/" + self.dataset + "_pre_train.emb"
        self.community_filename = "/content/CommunityGAN/data/" + self.app + "/" + self.dataset + ".sampled.cmty.txt"
        self.model_log = "/content/log/"
        self.cache_filename_prefix = "/content/cache/" + self.app + "/" + self.dataset
        self.emb_filenames = ["/content/results/" + self.app + "/" + self.dataset + "_gen_.emb",
                              "/content/results/" + self.app + "/" + self.dataset + "_dis_.emb"]
        self.result_filename = "/content/results/" + self.app + "/" + self.dataset + ".txt"

        # Calculate betweenness centrality and select top 70% nodes
        graph = nx.read_edgelist(self.train_filename)
        centrality_scores = nx.betweenness_centrality(graph)
        sorted_nodes = sorted(centrality_scores.items(), key=itemgetter(1), reverse=True)
        threshold = int(0.7 * len(sorted_nodes))
        top_nodes = sorted_nodes[:threshold]
        selected_node_ids = set(node_id for node_id, _ in top_nodes)

        # Create modified dataset file
        modified_train_filename = self.train_filename.replace("_train.txt", "_train_modified.txt")
        with open(self.train_filename, 'r') as original_file:
            with open(modified_train_filename, 'w') as modified_file:
                for line in original_file:
                    source, target = line.strip().split()
                    if source in selected_node_ids or target in selected_node_ids:
                        modified_file.write(line)

        # Update train filename
        self.train_filename = modified_train_filename